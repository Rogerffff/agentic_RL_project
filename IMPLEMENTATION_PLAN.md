...
- 当前方案已经覆盖 `SFT -> C-GRPO` 的训练主链路，但尚未补上 baseline 对照线、外部 benchmark 评测、以及最终结果汇报口径。
- 评测补充的目标不是一次性铺开论文全部 benchmark，而是先构造一个最小可比闭环：`SFT-only`、`Outcome-only GRPO`、`C-GRPO` 三条线统一在 `BrowseComp` 上比较。
- 首版项目验收标准是“能稳定产出可比较数字”，而不是“完整复现论文全部 benchmark”。
...
examples/
  ...
  carr_deepsearch/
    ...
    data_preprocess/
      ...
      preprocess_browsecomp.py    # BrowseComp -> verl parquet（评测集）
    config/
    ...
    scripts/
      ...
      run_eval_browsecomp.sh      # 基于 val_only 的 BrowseComp 评测脚本
...
## Phase 7.5: Baseline 与评测补充

### 7.5.1 评测范围与目标

首版评测范围固定为两层：

- **训练内验证**：继续使用 `DeepDive rl_val.parquet`
- **外部 benchmark**：只接入 `BrowseComp`

`DeepDive val` 的作用：

- 监控训练是否有效
- 对比 `outcome_reward`、`rubric_reward`、`num_turns`
- 辅助判断 C-GRPO 是否确实提升了推理完整性而不是仅靠运气答对

`BrowseComp` 的作用：

- 提供一个可写进简历的外部 benchmark 数字
- 统一比较 `SFT-only`、`GRPO`、`C-GRPO`

首版**暂不纳入**以下 benchmark：
...
原因：首版优先保证最小闭环，避免 benchmark 接入成本吞掉训练和验证时间。

### 7.5.2 GRPO Baseline 定义

首版 baseline 统一定义为三条线：

- **SFT-only**
  - 使用 SFT checkpoint
  - 通过 `run_eval_browsecomp.sh` 做纯评测
- **GRPO baseline**
  - 从同一个 SFT checkpoint 初始化
  - 使用与 C-GRPO 完全相同的数据、工具、rollout、judge、context 配置
  - 唯一区别是 `algorithm.adv_estimator=grpo`
- **C-GRPO**
  - 使用现有默认配置
  - 保持 `cgrpo_alpha=0.3`

GRPO baseline 训练命令约定：
...
- 现有 `carr_reward.py` 约定 `score = outcome_reward`
- 在标准 `GRPO` 下，advantage 只使用 `score`
- `rubric_reward` 虽会记录到日志，但不会参与优势计算
- 因此不需要为 GRPO baseline 再写一套奖励逻辑或额外新增 baseline config

### 7.5.3 BrowseComp 数据预处理方案

未来新增文件：`examples/carr_deepsearch/data_preprocess/preprocess_browsecomp.py`

输入：

- 数据源：`smolagents/browse_comp`

输出：

- `${oc.env:HOME}/data/carr_deepsearch/browsecomp_eval.parquet`

Schema 固定为与 RL parquet 对齐的最小子集：
...
> **关键区分：`prompt.content` 与 `search_forbidden_strs[0]` 的值不同。**
>
> - `prompt.content` = `problem + FORMAT_SUFFIX`（拼接后的完整 prompt）
> - `search_forbidden_strs[0]` = 原始 `problem` 文本（**不含** `FORMAT_SUFFIX`）
>
> 原因：CaRR reward server（`launch_server.py:480`）使用 `question = remote_env_info["search_forbidden_strs"][0]` 作为 LLM judge 的问题输入。如果此字段包含 `FORMAT_SUFFIX`，judge 会收到格式指令而非纯问题文本，导致 outcome reward 计算异常。

`FORMAT_SUFFIX` 固定沿用 DeepDive RL 的回答格式：
...
为什么 `rubric_reward_ratio=0.0` 可行：

- `CaRR` reward server 在 `ratio=0.0` 时只计算 outcome reward
- 会跳过 rubric reward
- 因而 `BrowseComp` 评测结果就是 LLM judge 下的 0/1 accuracy
- 这与论文的 benchmark 评测逻辑兼容
...
### 7.5.4 BrowseComp 评测脚本方案

未来新增文件：`examples/carr_deepsearch/scripts/run_eval_browsecomp.sh`

实现方式固定为：

- 底层使用 `python -m verl.trainer.main_ppo`
- 使用 `trainer.val_only=True`
- 复用完整 agent loop、tool server、reward loop、metric aggregation
- 不允许另写第二套 generation / eval 主程序

脚本参数契约固定为：

```bash
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <model_path> [max_samples] [context]
```

参数定义：

- `model_path`：待评测 checkpoint 路径
- `max_samples`：默认 `-1`，表示全量
- `context`：`64k` 或 `128k`，默认 `64k`

脚本内部必须写入的关键 override：

```bash
actor_rollout_ref.model.path="$MODEL_PATH"
data.train_files="$HOME/data/carr_deepsearch/rl_train.parquet"
data.val_files="$HOME/data/carr_deepsearch/browsecomp_eval.parquet"
...
- `main_ppo` 即使在 `val_only` 模式下也会初始化 train dataloader
- 因此 `data.train_files` 不能留空，必须继续指向 `rl_train.parquet`

`context -> max_response_length` 映射固定为：
...
训练内 `DeepDive val` 的主观察指标固定为：

- `val-core/carr_deepsearch/reward/mean@1`
- `val-aux/carr_deepsearch/outcome_reward/mean@1`
- `val-aux/carr_deepsearch/rubric_reward/mean@1`
- `val-aux/num_turns/mean`
- `response/aborted_ratio`

`BrowseComp` 正式对比指标固定为：

- `val-core/browsecomp/reward/mean@1`

说明：

- `BrowseComp` 评测时 `rubric_reward_ratio=0.0`
- 因而这里的 `reward/mean@1` 等价于 outcome accuracy

可选 sanity 指标：

- `val-aux/browsecomp/outcome_reward/mean@1`

简历汇报模板：

| Model | BrowseComp 64k | BrowseComp 128k |
|---|---:|---:|
| Qwen3-4B SFT | X.X% | X.X% |
| + GRPO | X.X% | X.X% |
| + C-GRPO | X.X% | X.X% |

辅助说明：

- `DeepDive val` 的 `rubric_reward/mean@1` 作为“推理完整性提升”的辅助证据
- `num_turns/mean` 与 `response/aborted_ratio` 作为训练稳定性与工具使用效率证据
...
- 代码仓库
- 可用 Python / CUDA 环境
- `SERPAPI_API_KEY`、`JINA_API_KEY`、`DEEPSEEK_API_KEY`
...
- `verl`、CaRR 依赖与 API Key 准备完成

验证标准:

- `python -c "import verl"` 成功
- `python -c "import aiohttp, quart"` 成功

### 8.1 数据预处理（SFT + RL + BrowseComp）

输入:

- `CaRR/data/deepdive-sft-glm46-trajectory-1k.jsonl`
- `CaRR/data/deepdive-rl-2k-rubrics.jsonl`
...
- `${HOME}/data/carr_deepsearch/sft_{train,val}.parquet`
- `${HOME}/data/carr_deepsearch/rl_{train,val}.parquet`
- `${HOME}/data/carr_deepsearch/browsecomp_eval.parquet`
...
# BrowseComp 数据检查
bc_df = pd.read_parquet(f"{__import__('os').path.expanduser('~')}/data/carr_deepsearch/browsecomp_eval.parquet")
assert all(bc_df["data_source"] == "browsecomp")
assert all(bc_df["agent_name"] == "carr_tool_agent")
assert all(bc_df["extra_info"].apply(lambda x: x["rubric_reward_ratio"] == 0.0))
```
...
- Tool server 正常监听 `7230`
- Reward server 正常监听 `8888`

验证标准:

- `smoke_test.py --tool` 通过
- `smoke_test.py --reward` 通过
- `smoke_test.py --all` 通过
...
- SFT checkpoint 保存到 `${HOME}/checkpoints/carr_deepsearch_sft/`
...
### 8.4 SFT 在 BrowseComp 上快速评测

输入:

- SFT checkpoint
- `browsecomp_eval.parquet`

命令:

```bash
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <SFT_CKPT_PATH> 100 64k
```

输出:

- 一次 100 条的 quick eval 结果
- `trainer.validation_data_dir` 下的验证输出

验证标准:

- 脚本可在 `val_only` 模式下完成
- 输出中包含 `val-core/browsecomp/reward/mean@1`
- accuracy 非 0，且服务链路无报错

### 8.5 GRPO baseline 训练

输入:

- 与 C-GRPO 相同的 `rl_train.parquet`
- 与 C-GRPO 相同的 SFT 初始化 checkpoint
...
- `${HOME}/checkpoints/carr_deepsearch_rl_grpo/` 下的 GRPO checkpoint

验证标准:

- 可完成至少一次小规模训练
- `outcome_reward` 非全零
- 无 NaN、reward 崩塌或持续性服务错误

### 8.6 GRPO 在 BrowseComp 上快速评测
...
```bash
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <GRPO_CKPT_PATH> 100 64k
```

输出:

- GRPO 的 BrowseComp quick eval 结果

验证标准:

- 输出包含 `val-core/browsecomp/reward/mean@1`
- 可与 SFT quick eval 结果直接横向对比
...
- `${HOME}/checkpoints/carr_deepsearch_rl/` 下的 C-GRPO checkpoint
...
```bash
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <CGRPO_CKPT_PATH> 100 64k
```

输出:

- C-GRPO 的 BrowseComp quick eval 结果

验证标准:

- 输出包含 `val-core/browsecomp/reward/mean@1`
- 至少能完成一次三模型 quick eval 的横向对比

### 8.9 全量 BrowseComp 与可选 128k 评测

输入:

- `browsecomp_eval.parquet`
- 三个最终 checkpoint：SFT / GRPO / C-GRPO

命令:

```bash
# 首版必做：全量 1270 条 + 64k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <SFT_CKPT_PATH> -1 64k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <GRPO_CKPT_PATH> -1 64k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <CGRPO_CKPT_PATH> -1 64k

# 第二轮增强项：全量 1270 条 + 128k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <SFT_CKPT_PATH> -1 128k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <GRPO_CKPT_PATH> -1 128k
bash examples/carr_deepsearch/scripts/run_eval_browsecomp.sh <CGRPO_CKPT_PATH> -1 128k
```

输出:

- 首版正式 benchmark 数字：`BrowseComp 64k`
- 可选增强数字：`BrowseComp 128k`

验证标准:

- 首版至少完成三模型的全量 `64k` 评测
- `128k` 与重复 3 次平均作为第二轮增强项，不阻塞首版收口

默认值锁定：

- 首版 quick eval：`100` 条
- 首版正式 eval：全量 `1270` 条 `64k`
- 可选增强：全量 `1270` 条 `128k`
- 可选增强：重复 `3` 次取平均
...
- 三模型的 BrowseComp 评测结果
- 训练内 `DeepDive val` 指标

产出:

- 一张三模型对比表：`SFT` / `GRPO` / `C-GRPO`
- 一组简历可直接使用的数字摘要

对比表至少包含以下列：

- `BrowseComp 64k`
- `BrowseComp 128k`（若已执行）
- `DeepDive val outcome_reward`
- `DeepDive val rubric_reward`
- `num_turns`
- `aborted_ratio`

验证标准:

- 最终产物不是口头结论，而是一张结构化对比表
- 至少能直接回答：`SFT < GRPO < C-GRPO` 是否成立
- 能直接抽取一条简历 bullet 所需的核心数字
...
| `val_only` 评测报 train dataloader 初始化错误 | 在 `run_eval_browsecomp.sh` 中显式设置 `data.train_files=$HOME/data/carr_deepsearch/rl_train.parquet` |
| BrowseComp parquet 字段与 Agent Loop 不兼容 | 预处理阶段固定复用 RL parquet schema，并在 `8.1` 增加字段级校验 |
