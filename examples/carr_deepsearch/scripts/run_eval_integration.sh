#!/bin/bash
# Manual integration eval launcher that keeps tool/reward servers alive long enough
# to capture tool cache stats and reward trace output after validation.
#
# Usage:
#   bash run_eval_integration.sh <model_path> <eval_file> <run_name> [extra hydra overrides...]
#
# Example:
#   bash run_eval_integration.sh \
#     /root/checkpoints/.../huggingface \
#     /root/verl-carr-deepsearch/examples/carr_deepsearch/data/rl_val_subset8_seed42.parquet \
#     deepdive_subset8_step591_64k_tp1_sample_t06_p095_k20_intg \
...
MODEL_PATH="${1:?Usage: run_eval_integration.sh <model_path> <eval_file> <run_name> [extra hydra overrides...]}"
EVAL_FILE="${2:?Usage: run_eval_integration.sh <model_path> <eval_file> <run_name> [extra hydra overrides...]}"
RUN_NAME="${3:?Usage: run_eval_integration.sh <model_path> <eval_file> <run_name> [extra hydra overrides...]}"
shift 3
...
CMD=(
  ...
  data.max_response_length="${CARR_MAX_RESPONSE_LENGTH:-61440}"
  data.val_batch_size="${CARR_VAL_BATCH_SIZE:-32}"
  ...
  actor_rollout_ref.rollout.tensor_model_parallel_size="${CARR_TP_SIZE:-1}"
  actor_rollout_ref.rollout.enforce_eager="${CARR_ENFORCE_EAGER:-true}"
  actor_rollout_ref.rollout.multi_turn.max_assistant_turns="${CARR_MAX_ASSISTANT_TURNS:-120}"
  actor_rollout_ref.rollout.multi_turn.max_tool_response_length="${CARR_MAX_TOOL_RESPONSE_LENGTH:-6000}"
  actor_rollout_ref.rollout.val_kwargs.n="${CARR_VAL_N:-1}"
  actor_rollout_ref.rollout.val_kwargs.temperature="${CARR_VAL_TEMPERATURE:-0.6}"
  actor_rollout_ref.rollout.val_kwargs.top_p="${CARR_VAL_TOP_P:-0.95}"
  actor_rollout_ref.rollout.val_kwargs.top_k="${CARR_VAL_TOP_K:-20}"
  actor_rollout_ref.rollout.val_kwargs.do_sample="${CARR_VAL_DO_SAMPLE:-true}"
  actor_rollout_ref.rollout.val_kwargs.presence_penalty="${CARR_VAL_PRESENCE_PENALTY:-0.0}"
  actor_rollout_ref.rollout.val_kwargs.frequency_penalty="${CARR_VAL_FREQUENCY_PENALTY:-0.0}"
  actor_rollout_ref.rollout.val_kwargs.repetition_penalty="${CARR_VAL_REPETITION_PENALTY:-1.0}"
  +actor_rollout_ref.rollout.custom.carr_budget.max_rollout_wall_time_s="${CARR_ROLLOUT_WALL_TIME_S:-360}"
  +actor_rollout_ref.rollout.custom.carr_budget.max_tool_calls="${CARR_MAX_TOOL_CALLS:-88}"
  +actor_rollout_ref.rollout.custom.carr_budget.max_search_calls="${CARR_MAX_SEARCH_CALLS:-40}"
  +actor_rollout_ref.rollout.custom.carr_budget.max_open_calls="${CARR_MAX_OPEN_CALLS:-32}"
  +actor_rollout_ref.rollout.custom.carr_budget.max_find_calls="${CARR_MAX_FIND_CALLS:-20}"
  "$ATTN_OVERRIDE"
